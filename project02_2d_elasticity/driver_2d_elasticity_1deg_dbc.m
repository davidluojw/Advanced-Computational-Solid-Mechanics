...
% exact_ux_yy = @(x,y) 0;
% 
% exact_uy    = @(x,y) -0.003 * y;
% exact_uy_x  = @(x,y) 0;
% exact_uy_y  = @(x,y) -0.003;
% exact_uy_xx = @(x,y) 0;
% exact_uy_xy = @(x,y) 0;
% exact_uy_yx = @(x,y) 0;
% exact_uy_yy = @(x,y) 0;
...
% Dirichlet BC
g1_x = @(x, y) exact_ux(x,y);
g1_y = @(x,y) exact_uy(x,y);
g2_x = @(x, y) exact_ux(x,y);
g2_y = @(x,y) exact_uy(x,y);
g3_x = @(x, y) exact_ux(x,y);
g3_y = @(x,y) exact_uy(x,y);
g4_x = @(x, y) exact_ux(x,y);
g4_y = @(x,y) exact_uy(x,y);
...
n_el_x = 40;               % number of element in x-direction
n_el_y = 40;               % number of element in y-direction
...
n_eq = (n_np - 2* n_np_y - 2 * n_np_x +4 ) * n_ed;   % number of equations
...
% find the node number of Dirichlet BC
g1_node_number = [];
g2_node_number = [];
g3_node_number = [];
g4_node_number = [];
for ny = 1 : n_np_y
    for nx = 1 : n_np_x
        ...
        if nx == 1
            g1_node_number(end+1) = AA;
        end
        if nx == n_np_x
            g2_node_number(end+1) = AA;
        end
        if ny == 1 && nx ~= 1 && nx ~= n_np_x
            g3_node_number(end+1) = AA;
        end
        if ny == n_np_y && nx ~= 1 && nx ~= n_np_x
            g4_node_number(end+1) = AA;
        end
    end
end
...
for ee = 1 : n_el
   ...
   for ll = 1 : n_int
     ...
     % Set up k_ele
     k_ele = k_ele + Bamat' * D * Bamat * detJ * weight(ll);
           ...
           if PP > 0
             ...
             for bb = 1 : n_en
                 for jj = 1 : n_ed
                     ...
                     elseif ismember(IEN(bb, ee), g1_node_number)
                         % do something for non-zero g boundary condition
                         F(PP) = F(PP) - k_ele(pp, qq) * g1{jj}(x_ele(bb), y_ele(bb));
                     elseif ismember(IEN(bb, ee), g2_node_number)
                         F(PP) = F(PP) - k_ele(pp, qq) * g2{jj}(x_ele(bb), y_ele(bb));
                     elseif ismember(IEN(bb, ee), g3_node_number)
                         F(PP) = F(PP) - k_ele(pp, qq) * g3{jj}(x_ele(bb), y_ele(bb));
                     elseif ismember(IEN(bb, ee), g4_node_number)
                         F(PP) = F(PP) - k_ele(pp, qq) * g4{jj}(x_ele(bb), y_ele(bb));
                     end  % end of if QQ
                 end % end of jj-loop
             end   % end of for bb
...
for ee = 1: n_el
  ...
  for aa = 1:n_en
      for ii = 1: n_ed
          ...
          elseif ismember(IEN(aa, ee), g1_node_number)
              disp(index) = g1{ii}(x_ele(aa), y_ele(aa));
          elseif ismember(IEN(aa, ee), g2_node_number)
              disp(index) = g2{ii}(x_ele(aa), y_ele(aa));
          elseif ismember(IEN(aa, ee), g3_node_number)
              disp(index) = g3{ii}(x_ele(aa), y_ele(aa));
          elseif ismember(IEN(aa, ee), g4_node_number)
              disp(index) = g4{ii}(x_ele(aa), y_ele(aa));
          end  % end of if QQ
      end
   end
end
...
for ii = 1 :n_ed:  n_np * n_ed
    xnew_coor(jj) = x_coor(jj) + displacement(ii);
    ynew_coor(jj) = y_coor(jj) + displacement(ii + 1);
    jj = jj + 1;
end
for ee = 1 : n_el
