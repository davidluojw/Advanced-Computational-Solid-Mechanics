...
n_el_x = 60;               % number of element in x-direction
n_el_y = 60;               % number of element in y-direction
n_el   = n_el_x * n_el_y;   % total number of element in 2D domain

n_en = 9; % number of  element nodes
...
for ey = 1 :  n_el_y
  for ex = 1 : n_el_x
    ee = (ey-1)*n_el_x + ex;
    IEN(1,ee) = (deg_eta * ey-2)* n_np_x + deg_xi * (ex - 1) + 1;
    IEN(2,ee) = (deg_eta * ey-2)* n_np_x + deg_xi * (ex - 1) + 3;
    IEN(3,ee) =  deg_eta * ey   * n_np_x + deg_xi * (ex - 1) + 3;
    IEN(4,ee) =  deg_eta * ey   * n_np_x + deg_xi * (ex - 1) + 1;
    IEN(5,ee) = (deg_eta * ey-2)* n_np_x + deg_xi * (ex - 1) + 2;
    IEN(6,ee) = (deg_eta * ey-1)* n_np_x + deg_xi * (ex - 1) + 3;
    IEN(7,ee) =  deg_eta * ey   * n_np_x + deg_xi * (ex - 1) + 2;
    IEN(8,ee) = (deg_eta * ey-1)* n_np_x + deg_xi * (ex - 1) + 1;
    IEN(9,ee) = (deg_eta * ey-1)* n_np_x + deg_xi * (ex - 1) + 2;
  end
end
