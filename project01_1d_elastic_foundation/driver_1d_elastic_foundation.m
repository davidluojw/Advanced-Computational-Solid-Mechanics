...
g = 1;
h = 0;
lambda=6.0; % foundation stiffness
p = sqrt(lambda);
% =========================================================================
...
for ee = 1 : n_el
    ...
    for ll = 1 : n_int
        ...
        Na_x = 0.0;
        for aa = 1 : n_en
            dx_dxi = dx_dxi + x_ele(aa) * ExponentialShape(aa, xi(ll), 1, p, hh );
            x_l = x_l + x_ele(aa) * ExponentialShape(aa, xi(ll), 0, p, hh );
        end
        dxi_dx = 1.0 / dx_dxi;
    
        for aa = 1 : n_en
            f_e(aa) = f_e(aa) + weight(ll) * ExponentialShape(aa, xi(ll), 0, p, hh ) * f(x_l) * dx_dxi;
            for bb = 1 : n_en
                k_e(aa,bb) = k_e(aa,bb) + weight(ll) * ExponentialShape(aa, xi(ll), 1, p, hh ) * ExponentialShape(bb, xi(ll), 1, p, hh ) * dxi_dx...
                    + lambda * weight(ll) * ExponentialShape(aa, xi(ll), 0, p, hh ) * ExponentialShape(bb, xi(ll), 0, p, hh ) * dx_dxi;
            end
        end
    end
...
for ee = 1 : n_el
    for ll = 1 : nqp
        ...
        for aa = 1 : n_en
            % x_l    = x_l    + x_ele(aa) * ExponentialShape(aa, xi(ll), 0, p, hh );
            % uh     = uh     + u_ele(aa) * ExponentialShape(aa, xi(ll), 0, p, hh );
            % dx_dxi = dx_dxi + x_ele(aa) * ExponentialShape(aa, xi(ll), 1, p, hh );
            % du_dxi = du_dxi + u_ele(aa) * ExponentialShape(aa, xi(ll), 1, p, hh );
